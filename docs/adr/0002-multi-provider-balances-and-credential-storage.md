# ADR-0002：多供应商余额与 Windows 凭据存储

- 状态：Accepted
- 日期：2026-08-14
- 适用版本：QuotaDock 0.6.0+

## 背景

QuotaDock 原先只有 Codex 1 周额度，一个全局快照与刷新锁无法准确表达多个数据源的
配置、部分成功、独立旧值和独立故障。DeepSeek 与 Kimi 余额查询需要 Bearer API Key，
把 Key 放入普通 JSON、前端 store 或日志会扩大泄漏面。Kimi API、Kimi 会员与 Kimi Code
还是互不相通的产品，不能合并口径。

## 决策

1. schema v5 以三个 `ProviderState` 为事实源，每个供应商独立保存配置状态、最新成功
...
4. v0.6.0 的 Kimi 范围仅为国内 API 开放平台。模型与 HTTP 端点不公开国际站分支，
   以后若正式支持必须另立决策、凭据账户和迁移方案。
5. DeepSeek/Kimi Key 由 Rust 后端写入 Windows Credential Manager，固定 service 为
   `com.rupingliu.quotadock`，账户分别为 `deepseek-api-key`、`kimi-cn-api-key`。
...
- 接受官方余额可能延迟；不宣称实时，不从差额推断交易、Tier 或消费者产品权益。
...
- 首版同时开放 Kimi 国际站：账户/Key/币种隔离，当前产品与迁移验证不足。
